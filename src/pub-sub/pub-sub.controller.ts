...
export class PubSubController {
    ...
    if (useConsistentJsendResp) {
      return res.status(201).send(
        new JSendResponseDto(
          ...
          { subscriberStatus, topicStatus, subscriptionDetails }
        )
      )
    } else {
      return res.status(201).send(
  ...
  */
  ...
  async getEventStatus(@Param('eventID') eventID: string, @Req() req: Request, @Res() res: Response) {
    ...
    let statusMessage: string = null, webhooksStatusesAndValues = null;
    if (latestEventStatus.statusCode != EventStatus.PROCESSING) {
      try {
          ...
          webhooksStatusesAndValues = require(fileNames[0]).webhooksStatusesAndValues;
          statusMessage = PubSubService.getEnumNameFromEnumValue(EventStatus, latestEventStatus.statusCode);
      } catch (err) {
        return res.status(500).send(new JSendResponseDto("failed", 500, 'Internal Error', "Plese try again or contact Pangaea"))
      }
    } else {
        statusMessage = PubSubService.getEnumNameFromEnumValue(EventStatus, latestEventStatus.statusCode);
    }
    return res.status(200).send(new JSendResponseDto(
        "success",
        200,
        `The details of the event with id of '${eventID}' have been successfully retrieved, kindly see it in data`,
        new EventStatusDto({ statusMessage, eventDetails: latestEventStatus, webhooksStatusesAndValues})
      )
    )
  }
}
